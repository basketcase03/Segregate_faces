...
def move_to_trump(image):
    src = testpath+'/'+image
    dest = dir_path+'/'+'segregated_trump_tree'
    shutil.copy(src, dest)

def move_to_non_trump(image):
    src = testpath+'/'+image
    dest = dir_path+'/'+'segregated_non_trump_tree'
    shutil.copy(src, dest)
...
def segregate(tree_model):
    ...
    for image in test_images:
        ...
        if(len(faceLoc)>0):
            ...
            result = tree_model.predict([encodeImg])
            if(result==1):
                move_to_trump(image)
        if(result!=1):
...
def main():
    df = create_dataset()
    tree_model = train_tree(df)
    segregate(tree_model)
